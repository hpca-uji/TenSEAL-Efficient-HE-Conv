...
class EncryptedTensor {
   ...
   protected:
    ...
    void dispatch_jobs(task_t& worker_func, size_t total_tasks) {
	    ...
	    printf("## Libreria tenseal fichero encrypted_tensor.h dispatch_jobs\n");
	    printf("## n_jobs vale %ld y total tasks %ld\n", n_jobs, total_tasks);
    ...
    /*----------------------------------------------------------------------- */
      ...
      void dispatch_jobs_im2row_channel(task_n3& worker_func, size_t n, size_t kernel, size_t stride, size_t c) {

        size_t total_tasks = pow((n - kernel + stride)/stride, 2) * c;
        size_t n_jobs = std::min(total_tasks, this->tenseal_context()->dispatcher_size());

        size_t batch_size = (total_tasks + n_jobs - 1) / n_jobs;

        printf("## n_jobs vale %ld, total tasks size es %ld, batch size es %ld\n", n_jobs, total_tasks, batch_size);
            
        vector<future<bool>> futures;
        for (size_t i = 0; i < total_tasks; i+=batch_size) {
                futures.push_back(
                this->tenseal_context()->dispatcher()->enqueue_task( 
                    worker_func, i, std::min(i + batch_size , total_tasks), batch_size));
        }
