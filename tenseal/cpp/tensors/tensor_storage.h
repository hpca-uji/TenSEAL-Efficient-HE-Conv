...
class TensorStorage {
   public:
    ...
    TensorStorage(const vector<vector<dtype_t>>& data, size_t H, size_t W) {
        printf("donde toca\n");
        vector<dtype_t> flat_data;
        flat_data.reserve(H * W);
    ...
    /*--------------------------------------------------------------------------------*/
     ...
     TensorStorage( size_t H, size_t W, size_t pad) {
        printf("tensor storage padding method\n");
        vector<dtype_t> flat_data;
        int H_pad = (H +2*pad);
        int W_pad = (W +2*pad);
        printf("flat_data: %d - %d\n", H_pad, W_pad);
        _data=_data;
    ...
    TensorStorage(const xt::xarray<dtype_t>& data){
        _data= data;
    }
    /**
    ...
    TensorStorage(const vector<vector<dtype_t>>& data,
        ...
        printf("dimension shape TensorStorage constructor: %ld y %ld\n", data[0].size(), shape[dim]);
        if (data[0].size() != shape[dim])
            throw invalid_argument("invalid dimension shape TensorStorage constructor");
        ...
        clock_t fin= clock();
        double time_taken = double(fin - inicio) / double(CLOCKS_PER_SEC);
        printf("---> bucle tensor storage: %f\n", time_taken);
        inicio=clock();
        _data = xt::adapt(flat_data, shape);
        fin= clock();
        time_taken = double(fin - inicio) / double(CLOCKS_PER_SEC);
        printf("---> adapt tensor storage: %f\n", time_taken);
    ...
    TensorStorage<dtype_t>& reverse_data() { 
        reverse(this->_data.begin(),this->_data.end());
        return *this;
        }
    /**
    ...
    TensorStorage<dtype_t> subscript(
        const vector<pair<size_t, size_t>>& pairs, vector<size_t> final_shape) {
        xt::xstrided_slice_vector slices;
        std::vector<size_t> shape = this->shape();
        int index = 0;
        for (auto& pair : pairs) {
        ...
        std::vector<dtype_t> dummy_data(_sliced_data.begin(),
                                        _sliced_data.end());
        return TensorStorage<dtype_t>(dummy_data, final_shape);
    }

    vector<dtype_t> subscript_vector(
        const vector<pair<size_t, size_t>>& pairs) {
        xt::xstrided_slice_vector slices;
        std::vector<size_t> shape = this->shape();
        int index = 0;
    ...
    xt::xarray<dtype_t> subscript_vector_xt_data(
        const vector<pair<size_t, size_t>>& pairs) {
        xt::xstrided_slice_vector slices;
        std::vector<size_t> shape = this->shape();
        int index = 0;
