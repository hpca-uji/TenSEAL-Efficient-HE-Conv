...
void replicate_vector(vector<T>& vec, size_t final_size) {
    ...
    //printf("replicate vector -> init_size: %ld - final_size: %ld, resta: %ld\n", init_size,final_size, final_size-init_size);
...
template <typename T, class Encoder, typename D>
shared_ptr<T> new_pack_vectors(const vector<shared_ptr<T>>& vectors) {
    size_t vectors_nb = vectors.size();
    size_t vector_size = vectors[0]->size();
    size_t output_size = vectors_nb * vector_size;
    printf("output_size: %ld - vectors_nb: %ld - vector_size: %ld\n", output_size, vectors_nb, vector_size);
    size_t slot_count = vectors[0]->tenseal_context()->template slot_count<Encoder>();
    ...
    //Ciphertext packed_vec = vectors[0]->ciphertext()[0];
    //packed_vec->chunked_size({slot_count});
    auto packed_vec = vectors[0]->copy();
    packed_vec->chunked_size({output_size});
    //return packed_vec;
    ...
    for (size_t i = 1; i < vectors_nb; i++) {
        // rotate vector then accumulate
        auto vec = vectors[i]->copy();
        vec->chunked_size({output_size});
        printf("vector_pos: %ld\n", vectors_nb-i);
        vec->rotate_inplace(-i*vector_size);
        packed_vec->add_inplace(vec);
    }
