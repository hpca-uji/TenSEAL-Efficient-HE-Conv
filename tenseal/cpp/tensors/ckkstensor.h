...
class CKKSTensor : public EncryptedTensor<double, shared_ptr<CKKSTensor>>,
   ...
   public:
    ...
    /*shared_ptr<CKKSTensor> matmul_plain_inplace_fila(
            const PlainTensor<double>& other) override;*/
    /*----------------------------------------------------------------------*/
   ...
   private:
    ...
    CKKSTensor(const shared_ptr<const CKKSTensor>& tensor,
                       const  xt::xarray<Ciphertext>& storage);

    CKKSTensor(const shared_ptr<const CKKSTensor>& tensor,  const int height, const int width, const int padding);
